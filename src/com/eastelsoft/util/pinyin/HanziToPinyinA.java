...
public class HanziToPinyinA {
    ...
    private static HanziToPinyinA sInstance;
    ...
    protected HanziToPinyinA(boolean hasChinaCollator) {
        mHasChinaCollator = hasChinaCollator;
    }

    public static HanziToPinyinA getInstance() {
        synchronized (HanziToPinyinA.class) {
            if (sInstance != null) {
                return sInstance;
            }
            ...
            for (int i = 0; i < locale.length; i++) {
                if (locale[i].equals(Locale.CHINA.getLanguage())) {
                    // Do self validation just once.
                    if (DEBUG) {
                        Log.d(TAG, "Self validation. Result: " + doSelfValidation());
                    }
                    sInstance = new HanziToPinyinA(true);
                    return sInstance;
                }
            }
