...
//		disableSnoozeAlert(context, alarmId);
	...
	private static ContentValues createContentValues(Alarm alarm) {
		...
		values.put(Alarm.Columns.THEDAY, alarm.theday);
	...
	static Calendar calculateAlarm(int hour, int minute,
		...
		String ctt = (String) DateFormat.format("yyyy-MM-dd hh-mm-ss", c);

		if (daysOfWeek.isRepeatSet()) {
			Calendar tt = Calendar.getInstance();
			tt.setTimeInMillis(System.currentTimeMillis());

			int nowHour = tt.get(Calendar.HOUR_OF_DAY);
			int nowMinute = tt.get(Calendar.MINUTE);
			...
			int addDays = daysOfWeek.getNextAlarm(tt);
			if (addDays > 0)
				tt.add(Calendar.DAY_OF_WEEK, addDays);
			c = tt;
			String vv = (String) DateFormat.format("yyyy-MM-dd hh-mm-ss", c);
	...
	public static Date strToDateTime(String arg0) {
		...
		try {
			resultDate = simpleDateFormat.parse(arg0.toString());
	...
	static String formatTime(final Context context, int hour, int minute,
			Alarm.DaysOfWeek daysOfWeek, String theday) {
		Calendar c = calculateAlarm(hour, minute, daysOfWeek, theday);
