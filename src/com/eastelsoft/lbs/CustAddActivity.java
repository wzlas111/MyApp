...
public class CustAddActivity extends BaseActivity {
	public static final String TAG = "CustAddActivity";
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// getString(R.string.data_reading));
	...
	private class OnBtLocationClickListenerImpl implements OnClickListener {
		public void onClick(View v) {
			try {
				// 获取定位位置
				new BaiduMapAction(CustAddActivity.this, amapCallback, "2")
						.startListener();
	...
	protected class OnBtCloseLokClickListenerImpl implements OnClickListener {
		public void onClick(View v) {
			...
			try {
				...
				CustAddActivity.this.openPopupWindowPG("");
	...
	protected class OnBtCloseLnoClickListenerImpl implements OnClickListener {
		public void onClick(View v) {
			...
			try {
				...
				Toast.makeText(CustAddActivity.this, "保存本地成功",
						Toast.LENGTH_SHORT).show();
				CustAddActivity.this.finish();
				...
				e.printStackTrace();
				Toast.makeText(CustAddActivity.this, "保存本地失败",
						Toast.LENGTH_SHORT).show();
		...
		public void handleMessage(Message msg) {
			try {
				...
				case 2:
					...
					// CustAddActivity.this.getApplicationContext(),
				...
				case 0:
					if ("1".equals(msg.obj.toString())) {
						...
						DBUtil.insertLCust(
								...
								"11", type_id, area_id, typeStr, areaStr, cjob, ctelephone2, ctelephone3, ctelephone4);
						sp = getSharedPreferences("userdata", 0);
						SetInfo set = IUtil.initSetInfo(sp);
						...
						Toast.makeText(
								CustAddActivity.this,
								getResources().getString(
										R.string.cust_upload_succ),
								Toast.LENGTH_SHORT).show();
						CustAddActivity.this.finish();
					} else if ("2".equals(msg.obj.toString())) {
						dialog(CustAddActivity.this, "客户名称与平台上重复，请更换客户名称!");

					} else {
						dialog(CustAddActivity.this,
								getResources().getString(
										R.string.cust_upload_err));
					}
					break;
				case 1:
					dialog(CustAddActivity.this, msg.obj.toString());
					break;
				case 11:
				...
				case 9:
					...
					if (location != null) {
						...
						new AMapAction(CustAddActivity.this, amapCallback, "")
								.startListener();
					}
					break;
				case 99:
	...
	private void displayLocation(Message msg, Location location) {
		try {
			CustAddActivity.this.lon = Util.format(location.getLongitude(),
					"#.######");
			CustAddActivity.this.lat = Util.format(location.getLatitude(),
					"#.######");
	...
	private void displayLocation(Message msg, SItude itude) {
		try {
			CustAddActivity.this.lon = Util.format(
					Double.parseDouble(itude.longitude), "#.######");
			CustAddActivity.this.lat = Util.format(
					Double.parseDouble(itude.latitude), "#.######");
			this.locationDesc = new BaseStationAction(CustAddActivity.this)
					.getLocation(itude);
			if (!"".equals(locationDesc)) {
				// tvInfoLocationDesc.setText(locationDesc);
				// tvInfoLocationDesc.setVisibility(View.VISIBLE);
	...
	class LocationThread implements Runnable {
		...
		public void run() {
			...
			try {
				if (location != null) {
					SItude itude = new BaseStationAction(CustAddActivity.this)
